...
class HRect {
	...
	protected static HRect infiniteHRect(int d) {
		...
		for (int i = 0; i < d; ++i) {
			vmin.coord[i] = new Integer(Integer.MIN_VALUE);
			vmax.coord[i] = new Integer(Integer.MAX_VALUE);
		}
	...
	protected HRect intersection(HRect r) {
		...
		for (int i = 0; i < min.coord.length; ++i) {
			newmin.coord[i] = (((Integer) min.coord[i])
					.compareTo((Integer) r.min.coord[i])) > 0 ? min.coord[i]
					: r.min.coord[i];
			newmax.coord[i] = (((Integer) max.coord[i])
					.compareTo((Integer) r.max.coord[i])) > 0 ? max.coord[i]
					: r.max.coord[i];

			if (((Integer) newmin.coord[i])
					.compareTo((Integer) newmax.coord[i]) >= 0)
				return null;
		}
	...
	protected double area() {
		...
		for (int i = 0; i < min.coord.length; ++i) {
			a *= (((Integer) max.coord[i]) - ((Integer) min.coord[i]));
		}
