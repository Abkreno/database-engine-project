...
class KDNode implements Serializable {
	...
	protected static KDNode srch(HPoint key, KDNode t, int K) {
		...
		for (int lev = 0; t != null; lev = (lev + 1) % K) {
			...
			if (!t.deleted && key.equals(t.k)) {
				return t;
			} else if (((Integer) key.coord[lev])
					.compareTo((Integer) t.k.coord[lev]) > 0) {
				t = t.right;
			} else {
				t = t.left;
			}
	...
	protected static void rsearch(HPoint lowk, HPoint uppk, KDNode t, int lev,
		...
		if (t == null)
			return;
		if (((Integer) lowk.coord[lev]).compareTo((Integer) t.k.coord[lev]) > 0) {
			rsearch(lowk, uppk, t.left, (lev + 1) % K, K, v);
		}
		int j;
		for (j = 0; j < K
				&& ((Integer) lowk.coord[lev])
						.compareTo((Integer) t.k.coord[lev]) <= 0
				&& ((Integer) uppk.coord[lev])
						.compareTo((Integer) t.k.coord[lev]) >= 0; j++)
			;
		if (j == K)
			v.add(t);
		if (((Integer) uppk.coord[lev]).compareTo((Integer) t.k.coord[lev]) > 0) {
	...
	protected static void nnbr(KDNode kd, HPoint target, HRect hr,
		...
		boolean target_in_left = ((Integer) target.coord[lev])
				.compareTo((Integer) pivot.coord[lev]) < 0;
